...
use crate::{
    adt::{queue::Queue, stack::Stack}, dict::{error::{TryDowngrade, TryUpgrade}, node::TryLink}, linked_list::{
        error::{TryAsMut, TryAsRef},
        list::LinkedList, node::NodePtr
    }
};
...
impl DictSet {
    pub fn new() -> Self {
        Self { root: DictNode::root(), count: 0 }
    }
    ...
    pub fn remove(&mut self, word: &str) -> Result<(), Box<dyn Error>> {
        let mut stack: Box<dyn Stack<NodePtr<DictNode>>> = Box::new(LinkedList::<NodePtr<DictNode>>::new());
        let mut ptr = self.root.clone();
        for c in word.chars() {
    ...
    pub fn link_suffixes(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        while let Some(node_ptr) = queue.poll() {
            ...
            if link.try_as_ref()?.has_child(state) {
                node_ptr.try_as_mut()?.suffix = Some(link.try_as_ref()?.get_child(state).try_downgrade()?);
            } else {
